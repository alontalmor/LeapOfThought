...
import argparse
import os
from LeapOfThought.artisets.soft_reasoning.rover import Rover
...
class TestExampleArtiSet:
    ...
    def test_build_artificial_dataset(self, variant):
        ...
        args = parse.parse_args(["-c", "Rover","-o", "create_artiset","-v", variant, \
                                 "-out","data/artiset_samples/Rover/sample.jsonl","--save_sample"])

        if not os.path.exists("data/artiset_samples/Rover"):
            os.mkdir('data/artiset_samples/Rover')
