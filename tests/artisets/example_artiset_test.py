...
import argparse
import os
from LeapOfThought.artisets.example_artiset import ExampleArtiSet

class TestExampleArtiSet:
    @pytest.mark.parametrize("variant", ("100", "100000"))
    def test_build_artificial_dataset(self, variant):
        ...
        args = parse.parse_args(["-c", "ExampleArtiset","-o", "create_artiset","-v", variant, \
                                 "-out","data/artiset_samples/ExampleArtiset/sample.jsonl","--save_sample"])

        if not os.path.exists("data/artiset_samples/ExampleArtiset"):
            os.mkdir('data/artiset_samples/ExampleArtiset')

        artiset = ExampleArtiSet(args)
