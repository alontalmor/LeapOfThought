...
import argparse
import os
from LeapOfThought.artisets.boolean_qa.sizes import Sizes
...
class TestExampleArtiSet:
    ...
    def test_build_artificial_dataset(self, variant):
        ...
        args = parse.parse_args(["-c", "Sizes","-o", "create_artiset","-v", variant, \
                                 "-out","data/artiset_samples/Sizes/sample.jsonl","--save_sample"])

        if not os.path.exists("data/artiset_samples/Sizes"):
            os.mkdir('data/artiset_samples/Sizes')
