...
import argparse
import os
from LeapOfThought.artisets.boolean_qa.twenty_questions import TwentyQuestions

class TestExampleArtiSet:
    @pytest.mark.parametrize("variant", (["it_replace_rand_split"]))
    def test_build_artificial_dataset(self, variant):
        ...
        args = parse.parse_args(["-c", "TwentyQuestions","-o", "create_artiset","-v", variant, \
                                 "-out","data/artiset_samples/TwentyQuestions/sample.jsonl","--save_sample"])

        if not os.path.exists("data/artiset_samples/TwentyQuestions"):
            os.mkdir('data/artiset_samples/TwentyQuestions')

        artiset = TwentyQuestions(args)
