...
CACHE_ROOT = Path(os.getenv('LEAPOFTHOUGHT_CACHE_ROOT', Path.home() / '.teachyourai'))
...
def upload_jsonl_to_s3(upload_path, instances, header=None, sample_indent=False):
    ...
    logger.info("Uploading to S3. Size of %s is %fMB" % (upload_path, float(os.stat(local_filename).st_size / 1000000)))
